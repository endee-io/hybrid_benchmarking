...
DATASET_CONFIG = {
    "scifact": {"hf_name": "BeIR/scifact-qrels", "split": "train", "out_dir": "scifact"},
    "quora":   {"hf_name": "BeIR/quora-qrels",   "split": "test",  "out_dir": "quora"},
}
...
def main():
    ...
    parser.add_argument("--dataset-name", choices=["scifact", "quora"], required=True,
                        help="Dataset name: 'scifact' or 'quora'")
    parser.add_argument("--output-base",  default="test",
                        help="Base output directory (default: test)")
    parser.add_argument("--testcycle",    required=False,
    ...
    if args.validation:
        output_dir = Path(args.output_base) / args.testcycle / f"concurrency{args.concurrency}"
