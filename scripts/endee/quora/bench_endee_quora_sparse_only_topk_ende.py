#!/usr/bin/env python3
"""
Endee BM25 Sparse-Only Benchmark Script
Runs query-only benchmarks across top-k values (endee_bm25 sparse mode).
...
VECTOR_TOKEN          = "mydb:MTlmVnzdJNEX4AuzMf0Zn0JULfmq3RKN"
INDEX_NAME            = "quora_bench_endee_bm25"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_bm25_sparse_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee BM25 Sparse Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Endee BM25 Sparse-Only Benchmark")
