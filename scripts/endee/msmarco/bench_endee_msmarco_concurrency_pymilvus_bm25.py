#!/usr/bin/env python3
"""
Endee Hybrid Benchmark Script — MSMARCO, Concurrency Sweep
...
COLLECTION_NAME  = "beir_msmarco_int8"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_msmarco_concurrency_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    ws = wb.active
    ws.title = "Endee MSMARCO Concurrency"
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee Hybrid Benchmark — {COLLECTION_NAME} (best of {RUNS_PER_CONCURRENCY} runs, top-k={TOP_K})"
    )
...
def main():
    print("=" * 60)
    print("Endee Hybrid Benchmark — MSMARCO Concurrency Sweep")
