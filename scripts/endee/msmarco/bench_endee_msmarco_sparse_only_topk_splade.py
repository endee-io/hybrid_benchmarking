#!/usr/bin/env python3
"""
Endee MSMARCO SPLADE Sparse-Only Benchmark Script
...
INDEX_NAME       = "msmarco_bench_splade_int16"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_msmarco_splade_sparse_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    ws = wb.active
    ws.title = "Endee SPLADE MSMARCO Sparse"
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee SPLADE MSMARCO Sparse Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Endee SPLADE MSMARCO Sparse-Only Benchmark")
