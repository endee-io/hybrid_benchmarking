#!/usr/bin/env python3
"""
Qdrant Hybrid Benchmark Script — MSMARCO, Top-K Sweep
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_qdrant_msmarco_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    title_cell = ws.cell(
        ...
        value=f"Qdrant Hybrid Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Qdrant Hybrid Benchmark — MSMARCO Top-K Sweep")
