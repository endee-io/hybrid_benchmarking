#!/usr/bin/env python3
"""
Endee MSMARCO Sparse-Only Benchmark Script (PyMilvus BM25)
...
INDEX_NAME       = "msmarco_bench_pymilvus_float32"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_msmarco_sparse_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee MSMARCO Sparse Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Endee MSMARCO Sparse-Only Benchmark")
