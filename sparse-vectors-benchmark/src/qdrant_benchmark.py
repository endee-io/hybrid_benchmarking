...
# Per-process Qdrant client cache (avoids re-initializing on every task)
_worker_client_cache = {}


def _get_or_init_worker_client(base_url):
    worker_pid = os.getpid()
    if worker_pid not in _worker_client_cache:
        _worker_client_cache[worker_pid] = QdrantClient(host=base_url, prefer_grpc=False)
        print(f"Worker {worker_pid} initialized Qdrant client for {base_url}")
    return _worker_client_cache[worker_pid]


async def _query_single(query_dict, worker_client, collection_name, vector_name, search_limit):
    loop = asyncio.get_event_loop()
    start = time.time()
    try:
        results = await loop.run_in_executor(
            None,
            lambda: worker_client.search(
...
async def _process_batch_async(queries, worker_client, collection_name, vector_name, search_limit, async_concurrency, batch_id):
    results = []
    worker_pid = os.getpid()
    pbar = tqdm(
        total=len(queries),
    ...
    for chunk_start in range(0, len(queries), async_concurrency):
        ...
        tasks = [_query_single(q, worker_client, collection_name, vector_name, search_limit) for q in chunk]
...
def _process_batch(batch_args):
    batch_id, queries, base_url, collection_name, vector_name, search_limit, async_concurrency = batch_args
    worker_client = _get_or_init_worker_client(base_url)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        return loop.run_until_complete(
            _process_batch_async(queries, worker_client, collection_name, vector_name, search_limit, async_concurrency, batch_id)
        )
    finally:
        loop.close()
...
def print_segment_info(base_url, collection_name):
    # get telemetry
    telemetry = requests.get(f"http://{base_url}:6333/telemetry?details_level=10").json()
...
@click.command()
@click.option('--base-url', default="localhost", help="The host of the Qdrant server")
...
@click.option('--upsert-batch-size', default=1000, help="Number of vectors per batch upserts")
@click.option('--parallel-batch-upsert', default=1, help="Number of parallel batch upserts")
...
def sparse_vector_benchmark(
    ...
    # gRPC client
    client = QdrantClient(host=base_url, prefer_grpc=True)
    ...
    print_segment_info(base_url, collection_name)

    # read queries and convert to picklable dicts
    query_data = read_sparse_matrix(query_data_file_path)
    query_count = query_data.shape[0]
    queries_list = []
    for i in range(query_count):
        point = query_data[i]
        sv = csr_to_sparse_vector(point)
        queries_list.append({"query_idx": i, "indices": sv.indices, "values": sv.values})
    ...
    for query_idx, results, duration_ms in all_query_results:
        ...
        if check_ground_truth and results:
            ...
            gt_id_set = set(expected_ids[:search_limit])
            recall = len(retrieved_ids & gt_id_set) / search_limit
            recalls.append(recall)

    print("---------------------------------------")
    if check_ground_truth and recalls:
        mean_recall = np.mean(recalls)
        print(f"Mean recall@{search_limit}: {round(mean_recall, 4)}")
    print("---------------------------------------")
    print("Search latency distribution:")
    if not latency:
        print("No queries completed — no latency data available.")
    ...
    if not latency:
        print("No latency data — skipping plot.")
        return
