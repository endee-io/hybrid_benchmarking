#!/usr/bin/env python3
"""
Qdrant Hybrid Benchmark Script — MSMARCO Splade, Top-K Sweep
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_qdrant_msmarco_splade_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    ws = wb.active
    ws.title = "Qdrant MSMARCO Splade Top-K"
    ...
    title_cell = ws.cell(
        ...
        value=f"Qdrant Splade Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Qdrant Splade Benchmark — MSMARCO Top-K Sweep")
