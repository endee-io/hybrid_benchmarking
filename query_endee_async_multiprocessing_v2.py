...
async def process_single_query(
    ...
    try:
        ...
        if search_results is None:
            ...
            return query_id, {}, {"query_id": query_id, "latency_ms": 0, "worker_id": worker_pid, "batch_id": batch_id, "error": "None result"}, {"query_id": query_id, "status": "error", "error": "None result", "worker_id": worker_pid, "batch_id": batch_id}
...
async def process_query_batch_async(
    ...
    for chunk_start in range(0, total_queries, async_concurrency):
        ...
        tasks = [process_single_query(q, index, top_k, batch_id, worker_pid) for q in chunk]
...
def process_query_batch(batch_data: Tuple) -> Dict[str, Any]:
    ...
    return loop.run_until_complete(
        process_query_batch_async(queries, index, top_k, batch_id, worker_pid, async_concurrency)
    )
...
def split_into_batches(
    ...
    batches = [
        (batch_id, queries[i : i + batch_size], top_k, index_name, vector_token, base_url, async_concurrency)
...
def main():
    ...
    parser.add_argument("--dataset-name",     type=str, required=True,       help="Dataset name, e.g. scifact")
    ...
    parser.add_argument("--async-concurrency", type=int, default=1,         help="Concurrent async queries per worker (default: 10)")
    parser.add_argument("--testcycle",         type=str, required=True,       help="Test cycle folder name (e.g. testcycle1)")
    ...
    parser.add_argument("--output-base",      type=str, default="test",      help="Base output directory (default: test)")
    args = parser.parse_args()

    output_dir = Path(args.output_base) / args.testcycle / f"concurrency{args.concurrency}"
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("Starting async query execution")
    ...
    logger.info("  Test cycle:        %s", args.testcycle)
    ...
    batches = split_into_batches(
        ...
        args.index_name, args.vector_token, args.base_url, args.async_concurrency
    )
    ...
    save_results(all_results, all_latencies, all_logs, output_dir, args.testcycle, args.concurrency, total_time)
