#!/usr/bin/env python3
"""
Endee MSMARCO Endee-BM25 Sparse-Only Benchmark Script
Runs query-only benchmarks across top-k values (sparse mode).
...
# CONFIGURATION
# ============================================================
BASE_URL             = "http://148.113.37.113:8080/api/v2"
VECTOR_TOKEN         = "mydb:u5uW8i3cjF5IHbuNXTWLFpgKUWzocRZn"
INDEX_NAME           = "msmarco_bench_endee_bm25_int16"
DATASET_NAME         = "beir_msmarco"
SPARSE_MODE          = "endee_bm25"
SPARSE_SCORING_MODEL = "endee_bm25"
DATA_DIR             = "data"
CONCURRENCY          = 16
CACHE_DIR            = "model_cache"
VALIDATION_VENV      = "validation-env"
PRECISION            = "int16"
SPARSE_MODEL         = "Endee BM25"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_msmarco_endee_bm25_sparse_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    ws = wb.active
    ws.title = "Endee BM25 MSMARCO Sparse"
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee BM25 MSMARCO Sparse Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Endee BM25 MSMARCO Sparse-Only Benchmark")
    print(f"Index      : {INDEX_NAME}")
    print(f"Sparse     : {SPARSE_MODE}")
