#!/usr/bin/env python3
"""
Endee Sparse-Only Benchmark Script
...
INDEX_NAME       = "quora_bench_pymilvus"
...
OUTPUT_EXCEL = os.path.join(
    WORK_DIR,
    f"bench_endee_sparse_topk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
)
...
def write_excel(rows: list, output_path: str):
    ...
    title_cell = ws.cell(
        ...
        value=f"Endee Sparse Benchmark — {INDEX_NAME} (best of {RUNS_PER_TOP_K} runs)"
    )
...
def main():
    print("=" * 60)
    print("Endee Sparse-Only Benchmark")
