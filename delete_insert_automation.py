...
from endee_delete_insert import EndeeDeleteInsert
...
def _run_indexing(
    ...
    """Delete old index then run endee_indexing_v2.py via subprocess to create + upload."""
    try:
    ...
    script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "endee_indexing_v2.py")
    if not os.path.isfile(script):
        raise FileNotFoundError(f"endee_indexing_v2.py not found at: {script}")
    testcycle = f"testcycle{combo_idx}"
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"endee_indexing_v2.py exited with code {proc.returncode}")
...
def _run_query_benchmark(
    ...
    """
    Run query_endee_async_multiprocessing_v2.py via subprocess (index-env / current Python).
    Returns the output directory path and any parsed latency summary.
    """
    script = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "query_endee_async_multiprocessing_v2.py")
    if not os.path.isfile(script):
        raise FileNotFoundError(f"query_endee_async_multiprocessing_v2.py not found at: {script}")
    testcycle  = f"testcycle{combo_idx}"
    output_dir = Path(output_base) / testcycle / f"concurrency{concurrency}"
    ...
    cmd = [
        ...
        "--async-concurrency", str(async_concurrency),
        "--testcycle",         testcycle,
        "--top-k",             str(top_k),
        "--output-base",       output_base,
    ]
...
def _run_validation(
    ...
    # correctness.json is saved inside output_base/testcycle/concurrency{n}/
    correctness_path = Path(output_base) / testcycle / f"concurrency{concurrency}" / "correctness.json"
...
def main():
    ...
    parser.add_argument("--dataset-name", required=True,
                        help="Dataset name, e.g. quora or scifact")
    ...
    parser.add_argument("--sparse-scoring-model", default="default",
                        help="Sparse scoring model passed to endee_indexing_v2.py (default: default)")
    ...
    parser.add_argument("--query-async-concurrency",  type=int, default=1,
                        help="Async queries per worker for query benchmark (default: 10)")
    ...
    # Validation
    parser.add_argument("--validation-env",      required=False,default="validation-env",
                        help="Path to validation virtualenv root (e.g. ~/validation-env). "
                             "Script will use <validation-env>/bin/python automatically.")
    parser.add_argument("--validation-cache-dir", default=None,
                        help="HuggingFace cache dir passed to endee_validation.py (optional)")

    # Operational
    parser.add_argument("--keep-testcycle", action="store_true", default=False,
                        help="Keep testcycle output folders after each combination (default: delete to save disk space)")
    ...
    validation_python = os.path.expanduser(
        os.path.join(args.validation_env, "bin", "python")
    )


    if not os.path.isfile(validation_python):
        print(f"ERROR: validation Python not found at '{validation_python}'\n"
              f"  Make sure --validation-env points to an existing virtualenv root "
    ...
    gt_npy_path = (Path(args.data_dir) / args.dataset_name /
                   f"{args.dataset_name}_ground_truth_ids.npy")
    gt_npy_exists = gt_npy_path.exists()
    try:
    ...
    # ── Load GT IDs (optional) ───────────────────────────────────────────────
    gt_ids: Optional[Set[str]] = None
    if gt_npy_exists:
        gt_ids = EndeeDeleteInsert.load_ground_truth_ids(args.data_dir, args.dataset_name)
        print(f"Loaded {len(gt_ids)} ground-truth IDs from '{gt_npy_path}'\n")
    else:
        print(f"No ground-truth npy found at '{gt_npy_path}' → "
              f"ground-truth/non-ground-truth combos will be skipped.\n"
              f"  Run: python embedding_creation_v2.py --dataset-id BeIR/{args.dataset_name} --ground-truth\n")
    ...
    print(f"  GT filter     : {'enabled (' + str(len(gt_ids)) + ' GT IDs)' if gt_ids else 'disabled'}")
    ...
    for combo_idx, (del_pct, del_pick, del_order, gt_filter, skip_insert, insert_order) in \
        ...
        query_result = _run_query_benchmark(
            ...
            async_concurrency=args.query_async_concurrency,
            top_k=args.top_k,
            combo_idx=combo_idx,
            output_base=args.query_output_base,
        )
        ...
        if not args.keep_testcycle:
            testcycle_path = Path(args.query_output_base) / f"testcycle{combo_idx}"
            if testcycle_path.exists():
                shutil.rmtree(testcycle_path)
                print(f"  Deleted testcycle folder: {testcycle_path}")
